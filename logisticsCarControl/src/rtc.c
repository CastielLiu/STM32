...
u8 RTC_Init(void)
{
	...
	{	 
		/*复位后对备份寄存器和RTC的访问被禁止，以下3行为使能访问  中文参考手册47页*/
	  	RCC->APB1ENR|=1<<28;     //使能电源时钟	    
		RCC->APB1ENR|=1<<27;     //使能备份时钟	    
		PWR->CR|=1<<8;           //取消备份区写保护
		
		RCC->BDCR|=1<<16;        //备份区域软复位	    复位后后备区数据将被清除
		RCC->BDCR&=~(1<<16);     //备份区域软复位结束	  	 
	    RCC->BDCR|=1<<0;         //开启外部低速振荡器     选择RTC时钟，除非备份区域软复位，否则不可修改。
		...
		RTC_Set(2018,9,25,15,36,10); //设置时间	  
		RTC->CRL&=~(1<<4);           //配置更新
		while(!(RTC->CRL&(1<<5)));   //等待RTC寄存器操作完成		 									  
		BKP->DR1=0X5050;  
	 	printf("FIRST TIME\n");
