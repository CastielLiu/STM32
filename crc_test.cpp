...
typedef  unsigned short  verify_width_t;



verify_width_t crcTable[256]={
...
int main()
{
	...
	while(!feof(fp))
	{
		fscanf(fp,"%x",&dataBuf[ii]);	
	//	printf("%x ",dataBuf[ii]);
		ii++;
	}
		
	
	verify_width_t temp =  crcCompute(dataBuf,100);
	
	printf("%x\r\n",temp);


	  //printf("sizeof(verify_width_t) =%d",sizeof(verify_width_t));
