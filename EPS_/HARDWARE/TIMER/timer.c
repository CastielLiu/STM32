...
u8 engine_status[]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
{ 
	static u16 ch1_rising_cnt=0; // 捕获到上升沿时的计数
	static u16 ch1_falling_cnt =0;// 捕获到下降沿时的计数
	static u8 capture_mode = 0;//上升沿捕获0  下降沿捕获1
	static u16 high_level_time = 0;
	static u16 low_level_time = 0;
	
	if (TIM_GetITStatus(TIM5, TIM_IT_CC1) != RESET)//通道1捕获中断
	{
		if(capture_mode==0)//捕获到上升沿
		{
			...
			if(ch1_rising_cnt>ch1_falling_cnt)
				low_level_time = ch1_rising_cnt - ch1_falling_cnt;//上升沿时刻  - 下降沿时刻=低电平时间
			else
				low_level_time = 65535-ch1_falling_cnt+ch1_rising_cnt;
		}
		else //捕获到下降沿
		{
			...
			if(ch1_rising_cnt < ch1_falling_cnt)
				high_level_time =  ch1_falling_cnt - ch1_rising_cnt;//下降沿时刻 - 上升沿时刻=高电平时间
			else
				high_level_time = 65535 + ch1_falling_cnt - ch1_rising_cnt;			
		}	
		if(capture_mode==1)//下一次捕获下降沿
		{
			duty_cycle = high_level_time/(high_level_time + low_level_time);
			//printf("duty_cycle = %f%%\r\n",duty_cycle*100);
